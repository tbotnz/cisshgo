...
func TestTranscriptReader_WithTemplate(t *testing.T) {
	fd := &FakeDevice{Hostname: "myrouter", Vendor: "cisco", Platform: "csr1000v"}
...
func TestTranscriptReader_MultipleFields(t *testing.T) {
	fd := &FakeDevice{Hostname: "sw1", Vendor: "cisco", Platform: "csr1000v"}
...
func TestTranscriptReader_InvalidTemplate(t *testing.T) {
	fd := &FakeDevice{}
	_, err := TranscriptReader("{{.Bad", fd)
	if err == nil {
		t.Error("expected error for invalid template")
	}
}

func TestTranscriptReader_ExecuteError(t *testing.T) {
	fd := &FakeDevice{}
	_, err := TranscriptReader("{{.NoSuchField | len}}", fd)
	if err == nil {
		t.Error("expected error for bad template execution")
	}
}
